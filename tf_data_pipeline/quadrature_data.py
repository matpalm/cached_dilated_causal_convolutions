    ...
    def wave(w):
        match w:
            case Waveform.TRIANGLE:
                # Phase-shift so triangle peak occurs at phase 0.
...
if __name__ == "__main__":
    ...
    parser.add_argument("--frequency-hz", type=float, default=440)
    ...
    data = []
    interps = [0, 0.1, 0.5, 0.9, 1]
    for interp in interps:
        data.append(
            calculate_interp_wave(
                opts.frequency_hz,
                opts.sample_rate_hz,
                opts.starting_phase,
                opts.num_samples,
    ...
    x = np.arange(len(data[0]["phase_sin"]))
    ...
    sns.lineplot(x=x, y=data[0]["phase_sin"], ax=ax_top, label="phase_sin")
    sns.lineplot(x=x, y=data[0]["phase_cos"], ax=ax_top, label="phase_cos")
    ax_top.set_ylabel("phase")
    ax_top.set_title("Quadrature Phase")

    for i, interp in enumerate(interps):  # in enumerate(["sine", "ramp"]):
        sns.lineplot(x=x, y=data[i]["wave"], ax=ax_bottom, label=str(interp))
    ax_bottom.set_ylabel("amplitude")
    ax_bottom.set_xlabel("sample index")
    ax_bottom.set_title("Derived Waves")

    fig.savefig("interp_waves.png")
